...
#ident "$Id: depmod.c 1.5 Tue, 27 Mar 2001 23:03:32 +1000 kaos $"
...
static void resolve(void)
{
	...
	for (i = n_modules - 1, mod = modules + i; i >= 0; --i, --mod) {
		...
		for (resolved = 1, n = 0, sym = mod->undefs.symtab;
			...
			if (psym->status == SYM_UNDEF && psym->length_nosuffix) {
				/* Try again ignoring the suffix */
				find = symhash[psym->hashval_nosuffix];
				while (find) {
					if (strncmp(find->name, psym->name,
					    psym->length_nosuffix) == 0 &&
					    !find->name[psym->length_nosuffix]) {
						psym->status = SYM_RESOLVED;
						psym->module = find->module;
					}
					find = find->next;
...
int DEPMOD_MAIN(int argc, char **argv)
{
	...
	while ((o = getopt_long(argc, argv, "aAb:C:eF:hnqsvVr",
		...
		case 'b':
#ifdef HAVE_REALPATH
			if (realpath(optarg, base_dir) == NULL)
			{
			    perror("Unable to canonicalise base directory, using as specified");
			    strncpy(base_dir, optarg, PATH_MAX - 1);
			}
#else
			strncpy(base_dir, optarg, PATH_MAX - 1);
#endif
